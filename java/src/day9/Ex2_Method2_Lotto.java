...
public class Ex2_Method2_Lotto {
	...
	public static void main(String[] args) {
		/* 앞 예제에서 사용한 메소드들을 필요하면 복사 붙여넣기해서 로또 프로그램을 작성하세요
	...
	/* 기능 : min~max 사이의 중복되지 않은 n개의 정수를 생성하여 배열로 반환하는 메소드
	...
	public static int[] createRandomArray(int min, int max, int n) {
		...
		//min=45, max=1 <- 이경우 min이랑 max를 바꿔줌
		if(min>max) {
			int tmp=min;
			min=max;
			max=tmp;
		}
		//min=1, max=5, n=6 <- 이경우 무조건 중복이나올수밖에없어서 오류
		if(max-min+1<n) {
			return null;
		}
		int arr[] = new int [n];
