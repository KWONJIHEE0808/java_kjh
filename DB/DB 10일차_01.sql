...
select se_name, count(bd_se_num) from seat
    ...
    left join bookdetail on bd_bo_num = bo_num and bd_se_num = se_num
    where se_use = 'Y'
	group by se_num;
    
-- 개봉일 기준으로 현재 상영중인 영화중 최신 영화 10개를 조회
select mo_open_date, mo_title from movie
	join screen on sc_mo_num = mo_num
    where sc_start_time >= now() and mo_open_date <= now()
    group by mo_num
	order by mo_open_date desc limit 10;
    
-- 서울에 있는 모든 영화관 정보를 조회
select th_name, th_region from theater
	where th_region = '서울';
...
select mo_title as 영화제목, sum(bo_amount) as 예매수 from movie
    ...
    group by mo_num
    order by bo_amount desc limit 10;
    
-- CGV에서 영화를 관람한 영화별 관람객수
